...
def lambda_handler(event, context):
    ...
    dynamodb = boto3.resource('dynamodb') # DynamoDBのクライアントオブジェクトを作成する
    table = dynamodb.Table('starwars-mania') # moviesテーブルを取得する

    # 動画を検索し、ページ単位でアイテム一覧を保存する。
    for items_per_page in search_videos('スターウォーズ'):
        # save_to_mongodb(collection, items_per_page)
        save_to_dynamodb(table, items_per_page)

    show_top_videos(table)  # ビュー数の多い動画を表示する。
...
def save_to_dynamodb(table, items):
    """
    DynamoDBにアイテムのリストを保存する
    """
    # 保存する前に、後で使いやすいようにアイテムを書き換える。
    for item in items:
...
def show_top_videos(table):
    """
    ビュー数の上位5件を表示する。
    """
    response = table.query(
        KeyConditionExepression=Key('id')
    )

    for item in response['Items']:
        print(item)
        # print(item['title'])
    # for item in table.find().sort('statistics.viewCount', DESCENDING).limit(5):
