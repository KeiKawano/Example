...
def fetch(url):
  ...
  f = urlopen(req)
  # HTTPヘッダからエンコーディングを取得する（明示されていない場合はutf-8とする）
...
def scrape(html):
  ...
  for partial_html in re.findall(r'a itemprop="url".*?<>\s*</a></li>', html, re.DOTALL):
    ...
    # 書籍のタイトルはitemprop=nameという属性を持つp要素から取得する
...
def save(db_path, books):
  """
    引数booksで与えらた書籍のリストをSQLiteデータベースに保存する
    データベースのパスは引数db_pathで与えられる
    戻り値：なし
  """

  conn = sqlite3.connect(db_path) # データベースを開、コネクションを取得する
  c = conn.cursor() # カーソルを取得する
  # execute()メソッドでSQL文を実行する
  # このスクリプトを何回実行しても同じ結果になるようにするため、booksテーブルが存在する場合あｈ削除する
